...
public class SlimeboundBlade : ImbuementWeaponItem
{
	...
	public override void OnHitNPC(Player player, NPC target, NPC.HitInfo hit, int damageDone)
	{
		...
		{
			Dust dust = Dust.NewDustPerfect(target.Center, DustID.BlueCrystalShard,
				Main.rand.NextVector2Circular(1.4f, 1.4f), 120, new Color(45, 230, 210), 0.7f);
			dust.noGravity = true;
		}
	}
		...
		Vector2 position, Vector2 velocity, int type, int damage, float knockback)
	{
		...
		for (int spread = firstSpread; spread <= lastSpread; spread++)
		{
			...
			Projectile.NewProjectile(source, position, ballVelocity, type, (int)(damage * 0.45f),
	...
	public override void ModifyTooltips(List<TooltipLine> tooltips)
	{
		...
		TooltipLine chargeLine = new(Mod, "GelVolley",
			$"Royal volley in {swingsUntilVolley} swing{(swingsUntilVolley == 1 ? string.Empty : "s")}")
		{
