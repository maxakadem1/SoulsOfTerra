...
/// <summary>
/// Reusable custom melee swing base. Weapons derive from this to customize timing, VFX, and impact without repeating swing logic.
/// </summary>
public abstract class BaseCustomSwingProjectile : ModProjectile
{
	...
	// Swing timing profile - override in derived class
	...
	// Visual and scale customization
	...
	// Arc definition - override for different swing paths
	protected virtual float GetWindupStartOffset(int direction) => -direction * 0.35f;
	protected virtual float GetWindupEndOffset(int direction) => -direction * 1.75f;
	protected virtual float GetSnapEndOffset(int direction) => direction * 1.15f;
	...
	// Customization points for derived projectiles
	...
	private float GetSwordAngle(int age, int direction)
	{
		...
		if (age < WindupEnd)
		{
			...
			float startOffset = GetWindupStartOffset(direction);
			float endOffset = GetWindupEndOffset(direction);
			float wobble = GetWindupWobble(windupProgress, direction);
			return MathHelper.Lerp(AimAngle + startOffset, AimAngle + endOffset, EaseInOut(windupProgress)) + wobble;
		}
		...
		if (age < SnapEnd)
		{
			...
			float startAngle = AimAngle + GetWindupEndOffset(direction);
			float endAngle = AimAngle + GetSnapEndOffset(direction);
			return MathHelper.Lerp(startAngle, endAngle, EaseOutCubic(snapProgress));
		}

		// Recovery hold
		return AimAngle + GetSnapEndOffset(direction);
	}
